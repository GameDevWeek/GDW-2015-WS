...
public class PlayerStateChangeEvent {
    public static interface Listener {
        void onPlayerStateChangeEvent(Entity entity, MovementComponent.State oldState, MovementComponent.State newState);
    }
    ...
    public static void emit(Entity entity, MovementComponent.State oldState, MovementComponent.State newState) {
        ...
        for (int i = 0, n = listeners.size; i < n; i++) {
            ((Listener) items[i]).onPlayerStateChangeEvent(entity, oldState, newState);
        }
        listeners.end();
    }
